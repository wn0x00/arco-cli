...
/**
 * Scaffold a new page directory under <root>/src/pages/<name> following
 * arco-design-pro's conventions: index.tsx + style/index.module.less +
 * locale/index.ts (single file with both en-US and zh-CN entries).
 *
 * Does not modify the project's src/routes.ts or src/locale/index.ts —
 * those edits are risky to automate across arco-design-pro variants and
 * presets, so we print the exact snippets for the user to paste instead.
 */
export default async function addPage(options: AddPageOptions): Promise<void> {
  const { intro, outro, log, note } = await clack();
  intro(chalk.bold('arco add page'));
  ...
  log.success(`Created ${chalk.cyan(relativePath)} (${options.type})`);

  note(result.routeSnippet, 'Register the route');
  note(result.menuSnippet, 'Add menu translation');
